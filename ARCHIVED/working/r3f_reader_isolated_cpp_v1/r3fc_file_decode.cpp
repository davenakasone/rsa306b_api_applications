...
) 
{
    ...
    for (long int ii = decode_start_byte; ii < decode_stop_byte; ii++)
    {
        ...
        switch (byte_getter)
        {
            case (0)    : snprintf(this->_holder, BUF_HOLDER-1, "<><><>%12ld)\t\t\t\t[NULL]    %4d\n", ii, byte_getter);         break;
            case (10)   : snprintf(this->_holder, BUF_HOLDER-1, "<><><>%12ld)\t\t\t\t[LF]      %4d\n", ii, byte_getter);         break;
            case (12)   : snprintf(this->_holder, BUF_HOLDER-1, "<><><>%12ld)\t\t\t\t[FF]      %4d\n", ii, byte_getter);         break;
            case (13)   : snprintf(this->_holder, BUF_HOLDER-1, "<><><>%12ld)\t\t\t\t[CR]      %4d\n", ii, byte_getter);         break;
            case (EOF)  : snprintf(this->_holder, BUF_HOLDER-1, "<><><>%12ld)\t\t\t\t[EOF]     %4d\n", ii, byte_getter);         break;
            default     : snprintf(this->_holder, BUF_HOLDER-1, "<><><>%12ld)\t%c    %4d\n", ii, this->_helper[0], byte_getter); break;
        }
