...
changes
	...
	https://stackoverflow.com/questions/60320840/numpy-fromstring-deprecated-use-frombuffer-instead
	https://numpy.org/doc/stable/reference/generated/numpy.frombuffer.html
	https://numpy.org/devdocs/reference/generated/numpy.fromstring.html
	r3f_reader.py:179: DeprecationWarning: The binary mode of fromstring is deprecated, as it behaves surprisingly on unicode inputs. Use frombuffer instead
...
If your r3f file contains more than 1 second of data, the script splits it into multiple 1 second-long .mat files. 
It can also parse and save the header data in a csv file and parse and save the footer data in a txt file. Code below.
...
class R3F:
	...
	def get_header_data(self):
		...
		self.chCorr.corrType = np.frombuffer(
	...
	def parse_footer(self, raw_footer):
		...
		footer.frameId = np.frombuffer(raw_footer[8:12],
		 dtype=np.uint32, count=1)
		footer.trig2Idx = np.frombuffer(raw_footer[12:14],
		 dtype=np.uint16, count=1)
		footer.trig1Idx = np.frombuffer(raw_footer[14:16],
		 dtype=np.uint16, count=1)
		footer.timeSyncIdx = np.frombuffer(raw_footer[16:18],
		 dtype=np.uint16, count=1)
