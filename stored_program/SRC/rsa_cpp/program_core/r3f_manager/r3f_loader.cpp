/*
    "r3f_manager_class"
    populating and loading the r3f file
...
)
{
    ...
    (void)this->_init();    // initialize the member variables
    ...
    if (this->cutil.h_matched_extension(r3f_input, "r3f") == false)   // verify extension
    {
        return this->cutil.report_status_code(CODEZ::_5_called_with_bad_paramerters);
    }
    if(this->cutil.h_find_bytes_in_file(r3f_input, this->_bytes_in_file) != CODEZ::_0_no_errors)    // check bytes
    {
        return this->cutil.report_status_code(CODEZ::_5_called_with_bad_paramerters);
    }
