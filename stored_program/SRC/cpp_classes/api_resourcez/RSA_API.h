...
#define true (1)
		...
		typedef enum
		{
			TraceTypeAverage = 0,
			TraceTypeMax     = 1,
			TraceTypeMaxHold = 2,    // don't use, causes and error
			TraceTypeMin     = 3,
			TraceTypeMinHold = 4     // don't use, causes and error
		} TraceType;
		...
		RSA_API_DLL ReturnStatus DPX_SetParameters
		(
			double           fspan, 
			double           rbw, 
			int32_t          bitmapWidth, 
			int32_t          tracePtsPerPixel, 
		...
		RSA_API_DLL ReturnStatus DPX_GetSogramHiResLine
		(
			int16_t* vData, 
			int32_t* vDataSize, 
			int32_t lineIndex, 
		...
		};    // enums for the special fileSuffixCtl values
		...
		enum 
		{ 
			IFSTRM_MAXTRIGGERS = 32    // max size of IFSTRMDATAINFO triggerIndices array 
		};
		enum 
		{
		...
		enum 
		{  // enums for IQSTREAM_SetDiskFilenameSuffix() suffixCtl values
		...
		enum 
		{ 
			IQSTRM_MAXTRIGGERS = 100    // max size of IFSTRMDATAINFO triggerIndices array 
		};
		enum 
		{
